...
const getConditionsByResort = async (req, res) => {
    ...
    try {
        const conditions = await Condition.find({ resort: resortId }).populate('user', 'username');
        if (!conditions.length) {
            return res.status(404).json({ message: "No condition reports found for this resort" });
        }
